...
import { Leaf, Users, Scale, RefreshCw, AlertTriangle, Sparkles } from "lucide-react";
...
export default function MaterialityStep1Page() {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["materiality-issues"] }); toast.success("AI 추천 점수가 적용되었습니다. 2단계에서 검토해 주세요."); },
  });
  ...
  const allMaterial = issues.filter((i) => getImpact(i) >= THRESHOLD || getFinancial(i) >= THRESHOLD);
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0">
            <div>
              ...
              <p className="text-sm text-muted-foreground">KPI 카탈로그 그룹 기반으로 {issues.length}개 이슈가 생성되었습니다. AI 추천 점수를 적용한 후 2단계로 진행하세요.</p>
            </div>
                          ...
                          return (
                            <div key={issue.id} className="flex items-center justify-between rounded border border-border/50 px-3 py-2 text-xs">
                              ...
                              <div className="flex items-center gap-2">
                                {hasScore ? (
                                  <>
                                    <span className="text-muted-foreground">영향 <strong className="text-foreground">{impact.toFixed(1)}</strong></span>
                                    <span className="text-muted-foreground">재무 <strong className="text-foreground">{financial.toFixed(1)}</strong></span>
                                  </>
                                ) : (
